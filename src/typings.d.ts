...
declare global {
  ...
  */
   ...
   * const el = GM_addElement('script', { src: 'https://example.com/script.js' });
   * el.onload = () => console.log('Script loaded:', el);
   *
   * @example
   * // Adding inline style
   * const styleEl = GM_addElement('style', { textContent: 'a { color: red; }' });
   *
   * @example
   * // Appending to a Shadow DOM
   * const iframeEl = GM_addElement(someElement.shadowRoot, 'iframe', { src: 'https://example.com' });
 */
